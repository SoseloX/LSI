...
class myICVLayer(nn.Module):

    def __init__(self, icv, lam):
        super(myICVLayer, self).__init__()
        self.icv = icv
        self.lam = lam
    ...
    def forward(self, x):
        if self.icv is not None:
            ...
            original_norm = torch.norm(x, p=2, dim=-1, keepdim=True)
            directions_all = []
            y = self.lam[0] * F.normalize(self.icv[0], dim=-1).repeat(1,x.shape[1],1)
            x = F.normalize(F.normalize(x.float(), p=2, dim=-1) + y, p=2, dim=-1) * original_norm
            return x.half()
        else:
            return x

class MeanICVLayer(nn.Module):

    def __init__(self, icv, lam):
        super(MeanICVLayer, self).__init__()
        self.icv = icv
        self.lam = lam
    ...
    def forward(self, x):
        if self.icv is not None:
            x = x.float()
            x = x + self.lam[0] * self.icv[0].repeat(1,x.shape[1],1)
            return x.half()
        else:
            return x
        

class MeanICVLayerNorm(nn.Module):

    def __init__(self, icv, lam):
        super(MeanICVLayerNorm, self).__init__()
        self.icv = icv
        self.lam = lam
    ...
    def forward(self, x):
        if self.icv is not None:
            x = x.float()
            non_zero_indices = (self.icv[0] != 0).nonzero(as_tuple=True)[0]
            icv_non_zero = self.icv[0][non_zero_indices]
            icv_norm = torch.norm(icv_non_zero, p=2, dim=-1)
            x_non_zero = x[:, :, non_zero_indices]
            x_norm = torch.norm(x_non_zero, p=2, dim=-1, keepdim=True)
            weight = x_norm / icv_norm
            x[:, :, non_zero_indices] = x[:, :, non_zero_indices] + self.lam[0] * icv_non_zero.repeat(1,x.shape[1],1) * weight
...
def add_my_icv_layers(model: PreTrainedModel, icv: Tensor, alpha: list):
    layers = get_layers(model)
    mlp_keywords = ["mlp", "feedforward", "ffn"]
    assert len(icv) == len(layers)
    for i, layer in enumerate(layers):
        original_mlp = find_module(layer, mlp_keywords)
        layer.mlp = nn.Sequential(original_mlp, myICVLayer(icv[i], alpha)) \

def add_mean_icv_layers(model: PreTrainedModel, icv: Tensor, alpha: list):
    layers = get_layers(model)
    mlp_keywords = ["mlp", "feedforward", "ffn"]
    assert len(icv) == len(layers)
    for i, layer in enumerate(layers):
        original_mlp = find_module(layer, mlp_keywords)
        layer.mlp = nn.Sequential(original_mlp, MeanICVLayer(icv[i], alpha)) 


def add_mean_norm_icv_layers(model: PreTrainedModel, icv: Tensor, alpha: list):
    layers = get_layers(model)
    mlp_keywords = ["mlp", "feedforward", "ffn"]
    assert len(icv) == len(layers)
    for i, layer in enumerate(layers):
        original_mlp = find_module(layer, mlp_keywords)
        layer.mlp = nn.Sequential(original_mlp, MeanICVLayerNorm(icv[i], alpha)) 
